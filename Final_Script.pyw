...
mixer.init()
alert=mixer.Sound('D:\CO2_LASER\Practice\Control_Panel\Final_Panel\Mario_Jumping.wav')
...
class Serial_Check (QThread):						#This thread continuously check the availability of comports and also revise the list of comport in main widget.
	...
	def listComportSer (self):				#for retrieving the list of comports from window port file 
		...
		try:
			...
			if count == 0:
				alert.play()
				QMessageBox.warning( self, "Attention", "No Comport is found!")
				count = 1
	...
	def run (self):
		...
		while True:
			...
			if new_error_csv != old_error_csv:
				alert.play()
				QMessageBox.warning( self, "Error in CSV File!", error)
...
class Pulse_Mode (QWidget):						#The class of pulse mode
	def __init__(self, parent):
		...
		global record
		self.duty, self.pwr = record.getData(1)
		...
		self.Power_Spin.setRange(self.pwr[0], self.pwr[7])
		self.Power_Spin.setValue(0)
		self.Power_Dial = QDial(self)			#Virtual Power Nobe of Power value
		self.Power_Dial.setRange((self.pwr[0]*10), (self.pwr[7]*10))
		...
		#self.connect (self.Load, SIGNAL("clicked()"), self.LoadData)
		#self.connect (self.Reset, SIGNAL("clicked()"), self.ResetData)
	...
	def UpdatePulse1_2 (self, val):
		...
		self.dutycycle = CubicSpline(x, y)(self.power)
		self.on = self.dutycycle * self.time_period / 100					#self.on is on time of pulse in Micro Seconds
		self.on_clock = int((self.on - 7.67) * 1000 / 62.5) - 1
	...
	#def LoadData (self):
		...
		#try:
			...
			#self.emit(SIGNAL("1"), str(self.on_clock))	
		#except BaseException as e:
...
class Continuous_Mode (QWidget):
	def __init__(self, parent):
		...
		Layout.addWidget (label, 0, 0, 1, 17)
		Layout.addLayout (control_struct, 1, 0, 10, 17)
		#Layout.addWidget (self.Load, 12, 6)
		#Layout.addWidget (self.Reset, 12, 7)
	...
	#def LoadData (self):
		#global ser
		#s = "M2;O"+str(int(self.on_clock))+";F"+str(int(self.off_clock))+";"
		#try:
			#ser.write (s.encode("utf-8"))
...
class Burst_Mode (QWidget):
	def __init__(self, parent):
		...
		self.Power_Label = QLabel ("<font size=5><b>POWER (Watt)</b></font>")
		self.Power_Spin = QDoubleSpinBox(self)
		self.Power_Spin.setRange(self.pwr[0], self.pwr[7]*10)
		self.Power_Spin.setValue(0)
		...
		#Layout.addWidget (self.Load, 12, 8)
		#Layout.addWidget (self.Reset, 12, 9)
	...
	def UpdatePulse2 (self, val):
		...
		self.dutycycle = CubicSpline(x, y)(self.power)
		self.on = self.dutycycle * self.time_period / 100					#self.on is on time of pulse in Micro Seconds
		self.on_clock = int((self.on - 8) * 1000 / 62.5) - 1

		self.off = self.time_period - self.on								#self.off is off time in Micro Seconds
		self.off_clock = int((self.off - 18)*1000 / 62.5) -1		
	...
	#def LoadData (self):
	#	global ser
	#	s = "M3;O"+str(int(self.on_clock))+";F"+str(int(self.off_clock))+";N"+str(self.bursts)
	#	try:
	#	ser.write (s.encode("utf-8"))
	#		self.emit(SIGNAL("2"),("On Clock is: "+str(int(self.on_clock))+" and "+"Off Clock is: "+str(int(self.off_clock))))
	#	except BaseException as e:
...
class Calibration_Window (QWidget):
	def __init__(self, parent):
		...
		self.connect (self.Next, SIGNAL("clicked()"), self.__next__)
	...
	def __next__ (self):
		...
		if (str(self.Power_Indicator.text()) == None):
			alert.play()
			QMessageBox.warning(self, "Value Error","<b>Power is mandatory!<b>")
		else:
			try:
				val = float (str(self.Power_Indicator.text()))
				self.next_count +=1
...
class Notification(QThread): 
	...
	def run(self):
		...
		if self.CurrentPort != self.OldPort:
			try:
				ser = serial.Serial (self.CurrentPort, 9600, timeout=5)
				time.sleep(2)#<------This Has to be checked whether it is required or not!
				ser.write("A".encode ("utf-8"))
				partner = ser.readline().decode("utf-8")
				if str(partner[0:4]) == "5000":
					self.OldPort = self.CurrentPort 
...
class Main_Widget (QWidget):
	...
	def __init__(self, parent):
		...
		#self.My_Label = QLabel ("<font color=blue size=6>Please select a Mode first!</b></font>")
	...
	def setPort (self):
		global ser
		self.Notes.getport(str(self.SerialPort.currentText()))
		self.Notes.start()
